...
BEGIN(Client)
class CEffect_MoonUFO_Laser_Burn final : public CInGameEffect
{
private:
	explicit CEffect_MoonUFO_Laser_Burn(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
	explicit CEffect_MoonUFO_Laser_Burn(const CEffect_MoonUFO_Laser_Burn& rhs);
	virtual ~CEffect_MoonUFO_Laser_Burn() = default; public:
...
public:
	void Set_Pos(_fvector vPos, _fvector vNormal);
...
public:
	static CEffect_MoonUFO_Laser_Burn* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext, void* pArg);
