...
class CEffect_Boss_BrokenLaser_Flow final : public CInGameEffect
{
private:
	explicit CEffect_Boss_BrokenLaser_Flow(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
	explicit CEffect_Boss_BrokenLaser_Flow(const CEffect_Boss_BrokenLaser_Flow& rhs);
	virtual ~CEffect_Boss_BrokenLaser_Flow() = default; public:
...
private:
	...
	const _float  m_fAlphaTime_Power = 0.f;
	const _float  m_fInstance_SpeedPerSec = 2.5f;
	const _double m_dInstance_Pos_Update_Time = 2.0;
	const _float2 m_vDefaultSize = { 0.2f, 0.6f };

private:
	_float3 m_vOffSet_Pos = { 0.f, -0.75f, -0.2f };

public:
	static CEffect_Boss_BrokenLaser_Flow* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext, void* pArg);
