...
BEGIN(Client)
class CEffect_StarBuddy_Explosion_Particle final: public CInGameEffect
{
private:
	explicit CEffect_StarBuddy_Explosion_Particle(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
	explicit CEffect_StarBuddy_Explosion_Particle(const CEffect_StarBuddy_Explosion_Particle& rhs);
	virtual ~CEffect_StarBuddy_Explosion_Particle() = default; public:
...
private:
	...
	const _float  m_fInstance_SpeedPerSec = 4.f;
	const _double m_dInstance_Pos_Update_Time = 1.0;
	const _float2 m_vDefaultSize = { 1.5f, 1.5f };

public:
	static CEffect_StarBuddy_Explosion_Particle* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext, void* pArg);
