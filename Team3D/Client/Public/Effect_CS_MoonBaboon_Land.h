...
BEGIN(Client)
class CEffect_CS_MoonBaboon_Land final : public CInGameEffect
{
private:
	explicit CEffect_CS_MoonBaboon_Land(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
	explicit CEffect_CS_MoonBaboon_Land(const CEffect_CS_MoonBaboon_Land& rhs);
	virtual ~CEffect_CS_MoonBaboon_Land() = default; public:
...
public:
	static CEffect_CS_MoonBaboon_Land* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext, void* pArg);
