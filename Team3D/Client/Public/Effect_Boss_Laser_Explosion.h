...
BEGIN(Client)
class CEffect_Boss_Laser_Explosion final : public CInGameEffect
{
private:
	explicit CEffect_Boss_Laser_Explosion(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
	explicit CEffect_Boss_Laser_Explosion(const CEffect_Boss_Laser_Explosion& rhs);
	virtual ~CEffect_Boss_Laser_Explosion() = default;
...
private:
	...
	_double m_dControlTime_Alpha = 1.2;
...
private:
	...
	const _float2 m_vDefaultSize = { 1.5f, 0.8f };

private:
	class CUFO* m_pUFO = nullptr;

public:
	static CEffect_Boss_Laser_Explosion* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext, void* pArg);
