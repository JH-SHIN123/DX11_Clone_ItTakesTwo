...
BEGIN(Client)
class CEffect_CS_UFO_Land final : public CInGameEffect
{
private:
	explicit CEffect_CS_UFO_Land(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
	explicit CEffect_CS_UFO_Land(const CEffect_CS_UFO_Land& rhs);
	virtual ~CEffect_CS_UFO_Land() = default; public:
...
public:
	static CEffect_CS_UFO_Land* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext, void* pArg);
