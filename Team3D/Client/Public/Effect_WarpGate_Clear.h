...
BEGIN(Client)
class CEffect_WarpGate_Clear final : public CInGameEffect
{
private:
	explicit CEffect_WarpGate_Clear(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
	explicit CEffect_WarpGate_Clear(const CEffect_WarpGate_Clear& rhs);
	virtual ~CEffect_WarpGate_Clear() = default; public:
...
public:
	static CEffect_WarpGate_Clear* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext, void* pArg);
