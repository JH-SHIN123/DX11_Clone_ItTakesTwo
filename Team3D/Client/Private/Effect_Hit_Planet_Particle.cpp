#include "stdafx.h"
#include "..\Public\Effect_Hit_Planet_Particle.h"
#include "DataStorage.h"
#include "Cody.h"

CEffect_Hit_Planet_Particle::CEffect_Hit_Planet_Particle(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	: CInGameEffect(pDevice, pDeviceContext)
{
}

CEffect_Hit_Planet_Particle::CEffect_Hit_Planet_Particle(const CEffect_Hit_Planet_Particle & rhs)
	: CInGameEffect(rhs)
{
}

HRESULT CEffect_Hit_Planet_Particle::NativeConstruct_Prototype(void * pArg)
{
...
HRESULT CEffect_Hit_Planet_Particle::NativeConstruct(void * pArg)
{
	if (nullptr != pArg)
...
_int CEffect_Hit_Planet_Particle::Tick(_double TimeDelta)
{
...
_int CEffect_Hit_Planet_Particle::Late_Tick(_double TimeDelta)
{
	return m_pRendererCom->Add_GameObject_ToRenderGroup(RENDER_GROUP::RENDER_EFFECT, this);
}

HRESULT CEffect_Hit_Planet_Particle::Render(RENDER_GROUP::Enum eGroup)
{
...
_float2 CEffect_Hit_Planet_Particle::Get_RandSize()
{
	_float2 vRandSize;
...
void CEffect_Hit_Planet_Particle::Check_Instance(_double TimeDelta)
{
	_float4 vMyPos;
	...
	{
		m_pInstanceBuffer_STT[iIndex].fTime -= (_float)TimeDelta * 0.9f;
		...
		m_pInstanceBuffer_STT[iIndex].vSize.x -= (_float)TimeDelta * 1.75f;
		m_pInstanceBuffer_STT[iIndex].vSize.y -= (_float)TimeDelta * 2.75f;
...
void CEffect_Hit_Planet_Particle::Instance_Size(_float TimeDelta, _int iIndex)
{
}
...
void CEffect_Hit_Planet_Particle::Instance_UV(_float TimeDelta, _int iIndex)
{
}

void CEffect_Hit_Planet_Particle::Reset_Instance(_double TimeDelta, _float4 vPos, _int iIndex)
{
}

HRESULT CEffect_Hit_Planet_Particle::Ready_InstanceBuffer()
{
...
_float3 CEffect_Hit_Planet_Particle::Get_Particle_Rand_Dir(_fvector vDefaultPos)
{
...
void CEffect_Hit_Planet_Particle::Check_Target_Matrix()
{
...
CEffect_Hit_Planet_Particle * CEffect_Hit_Planet_Particle::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext, void * pArg)
{
	CEffect_Hit_Planet_Particle*	pInstance = new CEffect_Hit_Planet_Particle(pDevice, pDeviceContext);
	if (FAILED(pInstance->NativeConstruct_Prototype(pArg)))
	{
		MSG_BOX("Failed to Create Instance - CEffect_Hit_Planet_Particle");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CEffect_Hit_Planet_Particle::Clone_GameObject(void * pArg)
{
	CEffect_Hit_Planet_Particle* pInstance = new CEffect_Hit_Planet_Particle(*this);
	if (FAILED(pInstance->NativeConstruct(pArg)))
	{
		MSG_BOX("Failed to Clone Instance - CEffect_Hit_Planet_Particle");
		Safe_Release(pInstance);
	}
	return pInstance;
}
