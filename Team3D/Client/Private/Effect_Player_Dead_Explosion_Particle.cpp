#include "stdafx.h"
#include "..\Public\Effect_Player_Dead_Explosion_Particle.h"

CEffect_Player_Dead_Explosion_Particle::CEffect_Player_Dead_Explosion_Particle(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	: CInGameEffect(pDevice, pDeviceContext)
{
}

CEffect_Player_Dead_Explosion_Particle::CEffect_Player_Dead_Explosion_Particle(const CEffect_Player_Dead_Explosion_Particle & rhs)
	: CInGameEffect(rhs)
{
}

HRESULT CEffect_Player_Dead_Explosion_Particle::NativeConstruct_Prototype(void * pArg)
{
...
HRESULT CEffect_Player_Dead_Explosion_Particle::NativeConstruct(void * pArg)
{
	if (nullptr != pArg)
	...
	FAILED_CHECK_RETURN(CGameObject::Add_Component(Level::LEVEL_STAGE, TEXT("Component_Texture_T_Dust_Motes"), TEXT("Com_Texture"), (CComponent**)&m_pTexturesCom), E_FAIL);
...
_int CEffect_Player_Dead_Explosion_Particle::Tick(_double TimeDelta)
{
	...
	m_fInstance_SpeedPerSec -= (_float)TimeDelta * (m_fInstance_SpeedPerSec * 0.75f);
...
_int CEffect_Player_Dead_Explosion_Particle::Late_Tick(_double TimeDelta)
{
	return m_pRendererCom->Add_GameObject_ToRenderGroup(RENDER_GROUP::RENDER_EFFECT, this);
}

HRESULT CEffect_Player_Dead_Explosion_Particle::Render(RENDER_GROUP::Enum eGroup)
{
...
void CEffect_Player_Dead_Explosion_Particle::Instance_Size(_float TimeDelta, _int iIndex)
{
}

void CEffect_Player_Dead_Explosion_Particle::Instance_Pos(_float TimeDelta, _int iIndex)
{
...
void CEffect_Player_Dead_Explosion_Particle::Instance_UV(_float TimeDelta, _int iIndex)
{
}

HRESULT CEffect_Player_Dead_Explosion_Particle::Ready_InstanceBuffer()
{
	...
	for (_int iIndex = 0; iIndex < iInstanceCount; ++iIndex)
	{
		...
		m_pInstanceBuffer_STT[iIndex].vTextureUV = __super::Get_TexUV_Rand(2, 2);
...
_float2 CEffect_Player_Dead_Explosion_Particle::Get_Rand_Size()
{
...
void CEffect_Player_Dead_Explosion_Particle::Check_InstanceBuffer(_double TimeDelta)
{
	_int iInstanceCount = m_EffectDesc_Prototype.iInstanceCount;
	for (_int iIndex = 0; iIndex < iInstanceCount; ++iIndex)
	{
...
CEffect_Player_Dead_Explosion_Particle * CEffect_Player_Dead_Explosion_Particle::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext, void * pArg)
{
	CEffect_Player_Dead_Explosion_Particle*	pInstance = new CEffect_Player_Dead_Explosion_Particle(pDevice, pDeviceContext);
	if (FAILED(pInstance->NativeConstruct_Prototype(pArg)))
	{
		MSG_BOX("Failed to Create Instance - CEffect_Player_Dead_Explosion_Particle");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CEffect_Player_Dead_Explosion_Particle::Clone_GameObject(void * pArg)
{
	CEffect_Player_Dead_Explosion_Particle* pInstance = new CEffect_Player_Dead_Explosion_Particle(*this);
	if (FAILED(pInstance->NativeConstruct(pArg)))
	{
		MSG_BOX("Failed to Clone Instance - CEffect_Player_Dead_Explosion_Particle");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void CEffect_Player_Dead_Explosion_Particle::Free()
{
