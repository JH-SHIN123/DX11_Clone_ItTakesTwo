...
HRESULT CEffect_Env_Particle_Field_Dust::Render(RENDER_GROUP::Enum eGroup)
{
	...
	m_pPointInstanceCom_STT->Set_ShaderResourceView("g_ColorTexture", m_pTexturesCom_Second->Get_ShaderResourceView(5));
	m_pPointInstanceCom_STT->Render(14, m_pInstanceBuffer_STT, m_EffectDesc_Prototype.iInstanceCount);

	return S_OK;
}

void CEffect_Env_Particle_Field_Dust::Check_State(_double TimeDelta)
{
...
void CEffect_Env_Particle_Field_Dust::State_Start(_double TimeDelta)
{
...
void CEffect_Env_Particle_Field_Dust::State_Disappear(_double TimeDelta)
{
	_float fTimeDelta = (_float)TimeDelta;
	_fvector vWorldPos = m_pTransformCom->Get_State(CTransform::STATE_POSITION);
