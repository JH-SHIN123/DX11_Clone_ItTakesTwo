...
CEffect_Boss_Gravitational_Bomb_Particle::CEffect_Boss_Gravitational_Bomb_Particle(const CEffect_Boss_Gravitational_Bomb_Particle & rhs)
	: CInGameEffect(rhs)
{
}

HRESULT CEffect_Boss_Gravitational_Bomb_Particle::NativeConstruct_Prototype(void * pArg)
{
...
HRESULT CEffect_Boss_Gravitational_Bomb_Particle::NativeConstruct(void * pArg)
{
	if (nullptr != pArg)
...
_int CEffect_Boss_Gravitational_Bomb_Particle::Late_Tick(_double TimeDelta)
{
...
void CEffect_Boss_Gravitational_Bomb_Particle::Instance_Size(_float TimeDelta, _int iIndex)
{
}

void CEffect_Boss_Gravitational_Bomb_Particle::Instance_Pos(_float TimeDelta, _int iIndex)
{
...
void CEffect_Boss_Gravitational_Bomb_Particle::Instance_UV(_float TimeDelta, _int iIndex)
{
}
...
_float3 CEffect_Boss_Gravitational_Bomb_Particle::Get_Particle_Rand_Dir(_fvector vDefaultPos)
{
	_vector vRandDir = XMLoadFloat3(&__super::Get_Dir_Rand(_int3(100, 100, 100)));
...
CEffect_Boss_Gravitational_Bomb_Particle * CEffect_Boss_Gravitational_Bomb_Particle::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext, void * pArg)
{
	CEffect_Boss_Gravitational_Bomb_Particle*	pInstance = new CEffect_Boss_Gravitational_Bomb_Particle(pDevice, pDeviceContext);
	if (FAILED(pInstance->NativeConstruct_Prototype(pArg)))
	{
		MSG_BOX("Failed to Create Instance - CEffect_Boss_Gravitational_Bomb_Particle");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CEffect_Boss_Gravitational_Bomb_Particle::Clone_GameObject(void * pArg)
{
	CEffect_Boss_Gravitational_Bomb_Particle* pInstance = new CEffect_Boss_Gravitational_Bomb_Particle(*this);
	if (FAILED(pInstance->NativeConstruct(pArg)))
	{
		MSG_BOX("Failed to Clone Instance - CEffect_Boss_Gravitational_Bomb_Particle");
		Safe_Release(pInstance);
	}
	return pInstance;
}
