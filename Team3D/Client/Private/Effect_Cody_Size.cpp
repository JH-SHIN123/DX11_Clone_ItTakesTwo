...
HRESULT CEffect_Cody_Size::Render()
{
	SetUp_Shader_Data();

	_int iInstanceCount = Get_InstanceCount();
	_float4 vColor = { 0.498039246f, 1.000000000f, 0.831372619f, 1.000000000f };
	...
	m_pPointInstanceCom->Render(1, m_pInstanceBuffer, iInstanceCount - 1);
...
void CEffect_Cody_Size::Change_SizeUp()
{
	m_IsReSizing = true;
	m_EffectDesc_Prototype.fLifeTime = 2.f;

	switch (m_eChangeSize_Cur)
	{
	case Client::CEffect_Cody_Size::TYPE_SMALL:
		m_eChangeSize_Next = TYPE_MIDDLE;
		break;
	case Client::CEffect_Cody_Size::TYPE_MIDDLE:
		m_eChangeSize_Next = TYPE_LARGE;
		break;
	default:
		m_IsReSizing = false;
		break;
...
void CEffect_Cody_Size::Change_SizeDown()
{
	m_IsReSizing = true;
	m_EffectDesc_Prototype.fLifeTime = 2.f;
	...
	{
	case Client::CEffect_Cody_Size::TYPE_MIDDLE:
		m_eChangeSize_Next = TYPE_SMALL;
		break;
	case Client::CEffect_Cody_Size::TYPE_LARGE:
		m_eChangeSize_Next = TYPE_MIDDLE;
		break;
	default:
		m_IsReSizing = false;
...
void CEffect_Cody_Size::Check_Size(_double TimeDelta)
{
	...
	{
		Check_Resizing_End(TimeDelta);
		return;
	}

	if (m_eChangeSize_Cur < m_eChangeSize_Next) // 커져라
	{
		switch (m_eChangeSize_Next)
		{
		case Client::CEffect_Cody_Size::TYPE_MIDDLE:
	...
	else if (m_eChangeSize_Cur > m_eChangeSize_Next) // 작아져라
	{
		switch (m_eChangeSize_Next)
		{
		case Client::CEffect_Cody_Size::TYPE_SMALL:
...
void CEffect_Cody_Size::Resizing_MS(_double TimeDelta)
{
	...
	_float fSize = (XMVector3Length(WorldMatrix.r[0])).m128_f32[0];
...
void CEffect_Cody_Size::Resizing_LM(_double TimeDelta)
{
	...
	for (_int iIndex = 0; iIndex < m_iInstance_Middle; ++iIndex)
	{
		_vector vWorldPos = XMLoadFloat4(&m_pInstance_LocalPos[iIndex]);
		vWorldPos = XMVector3Transform(vWorldPos, WorldMatrix);
		XMStoreFloat4(&m_pInstanceBuffer[iIndex].vPosition, vWorldPos);
...
void CEffect_Cody_Size::Check_Resizing_End(_double TimeDelta)
{
	if (TYPE_MIDDLE == m_eChangeSize_Cur && TYPE_SMALL == m_eChangeSize_Next)
		Resizing_MS_End(TimeDelta);

	else if (TYPE_LARGE == m_eChangeSize_Cur && TYPE_MIDDLE == m_eChangeSize_Next)
		Resizing_LM_End(TimeDelta);

	else if (TYPE_SMALL == m_eChangeSize_Cur && TYPE_MIDDLE == m_eChangeSize_Next)
		Resizing_SM_End(TimeDelta);

	else if (TYPE_MIDDLE == m_eChangeSize_Cur && TYPE_LARGE == m_eChangeSize_Next)
		Resizing_ML_End(TimeDelta);

	m_eChangeSize_Cur = m_eChangeSize_Next;
}
