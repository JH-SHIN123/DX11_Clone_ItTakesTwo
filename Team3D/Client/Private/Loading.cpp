...
HRESULT CLoading::LoadingForStage(_uint iThreadIndex)
{
	...
	else if (1 == iThreadIndex)
	{
		...
		CEnvironment_Generator::GetInstance()->Load_Environment_Model_Prototype(m_pDevice, m_pDeviceContext);
		CEnvironment_Generator::GetInstance()->Load_Environment_GameObject_Prototype(m_pDevice, m_pDeviceContext);
