...
HRESULT CLoading::LoadingForStage(_uint iThreadIndex)
{
	...
	else if (1 == iThreadIndex)
	{
		...
		CEffect_Generator::GetInstance()->Load_EffectData(TEXT("../Bin/Data/Effect/Stage1_Effect.dat"), m_pDevice, m_pDeviceContext);
	}
	else if (2 == iThreadIndex)
	{
		...
		FAILED_CHECK_RETURN(m_pGameInstance->Add_Component_Prototype(Level::LEVEL_STAGE, TEXT("Component_Model_TileBox"), CModel_Instance::Create(m_pDevice, m_pDeviceContext, 10000, "../Bin/Resources/Test/Mesh/DoorWay/", "DoorWay.fbx", TEXT("../Bin/ShaderFiles/Shader_MeshInstance.hlsl"), "DefaultTechnique", PivotMatrix)), E_FAIL);
		FAILED_CHECK_RETURN(m_pGameInstance->Add_GameObject_Prototype(Level::LEVEL_STAGE, TEXT("GameObject_TileBox"), CTest_EnvObject::Create(m_pDevice, m_pDeviceContext)), E_FAIL);
	}
