#include "stdafx.h"
#include "..\Public\Effect_CS_UFO_Land.h"
#include "DataStorage.h"
#include "UFO.h"

CEffect_CS_UFO_Land::CEffect_CS_UFO_Land(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	: CInGameEffect(pDevice, pDeviceContext)
{
}

CEffect_CS_UFO_Land::CEffect_CS_UFO_Land(const CEffect_CS_UFO_Land & rhs)
	: CInGameEffect(rhs)
{
}

HRESULT CEffect_CS_UFO_Land::NativeConstruct_Prototype(void * pArg)
{
...
HRESULT CEffect_CS_UFO_Land::NativeConstruct(void * pArg)
{
	if (nullptr != pArg)
...
_int CEffect_CS_UFO_Land::Tick(_double TimeDelta)
{
...
_int CEffect_CS_UFO_Land::Late_Tick(_double TimeDelta)
{
	return m_pRendererCom->Add_GameObject_ToRenderGroup(RENDER_GROUP::RENDER_EFFECT_NO_BLUR, this);
}

HRESULT CEffect_CS_UFO_Land::Render(RENDER_GROUP::Enum eGroup)
{
	m_pPointInstanceCom_STT->Set_DefaultVariables();

	_float4 vColor = { 2.f, 2.f, 2.f, 1.f };
...
void CEffect_CS_UFO_Land::Instance_Size(_float TimeDelta, _int iIndex)
{
}

void CEffect_CS_UFO_Land::Instance_Pos(_float TimeDelta, _int iIndex)
{
...
void CEffect_CS_UFO_Land::Instance_UV(_float TimeDelta, _int iIndex)
{
...
HRESULT CEffect_CS_UFO_Land::Ready_InstanceBuffer()
{
...
_float2 CEffect_CS_UFO_Land::Get_Rand_Size()
{
...
void CEffect_CS_UFO_Land::Check_InstanceBuffer(_double TimeDelta)
{
	_int iInstanceCount = m_EffectDesc_Prototype.iInstanceCount;
	for (_int iIndex = 0; iIndex < iInstanceCount; ++iIndex)
	{
...
void CEffect_CS_UFO_Land::Check_WorldMatrix()
{
	_matrix WorldMatrix = static_cast<CUFO*>(DATABASE->Get_BossUFO())->Get_Transform()->Get_WorldMatrix();
	_matrix BoneMatrix  = static_cast<CUFO*>(DATABASE->Get_BossUFO())->Get_Model()->Get_BoneMatrix("Base");

	BoneMatrix.r[3].m128_f32[1] -= 2.f;
...
CEffect_CS_UFO_Land * CEffect_CS_UFO_Land::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext, void * pArg)
{
	CEffect_CS_UFO_Land*	pInstance = new CEffect_CS_UFO_Land(pDevice, pDeviceContext);
	if (FAILED(pInstance->NativeConstruct_Prototype(pArg)))
	{
		MSG_BOX("Failed to Create Instance - CEffect_CS_UFO_Land");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CEffect_CS_UFO_Land::Clone_GameObject(void * pArg)
{
	CEffect_CS_UFO_Land* pInstance = new CEffect_CS_UFO_Land(*this);
	if (FAILED(pInstance->NativeConstruct(pArg)))
	{
		MSG_BOX("Failed to Clone Instance - CEffect_CS_UFO_Land");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void CEffect_CS_UFO_Land::Free()
{
