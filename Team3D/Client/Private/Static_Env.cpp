...
HRESULT CStatic_Env::NativeConstruct(void * pArg)
{
	...
	FAILED_CHECK_RETURN(CGameObject::Add_Component(Level::LEVEL_STAGE, TEXT("Component_StaticActor"), TEXT("Com_Actor"), (CComponent**)&m_pStaticActorCom, &CStaticActor::ARG_DESC(m_pTransformCom->Get_WorldMatrix(), &PxTriangleMeshGeometry(pMesh), m_pGameInstance->Create_PxMaterial(0.5f, 0.5f, 0.f), "Test")), E_FAIL);
	pMesh->release();
