#include "stdafx.h"
#include "..\Public\Effect_Boss_BrokenLaser_Lightning.h"
#include "DataStorage.h"
#include "UFO.h"

CEffect_Boss_BrokenLaser_Lightning::CEffect_Boss_BrokenLaser_Lightning(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	: CInGameEffect(pDevice, pDeviceContext)
{
}

CEffect_Boss_BrokenLaser_Lightning::CEffect_Boss_BrokenLaser_Lightning(const CEffect_Boss_BrokenLaser_Lightning & rhs)
	: CInGameEffect(rhs)
{
}

HRESULT CEffect_Boss_BrokenLaser_Lightning::NativeConstruct_Prototype(void * pArg)
{
...
HRESULT CEffect_Boss_BrokenLaser_Lightning::NativeConstruct(void * pArg)
{
	if (nullptr != pArg)
...
_int CEffect_Boss_BrokenLaser_Lightning::Tick(_double TimeDelta)
{
...
_int CEffect_Boss_BrokenLaser_Lightning::Late_Tick(_double TimeDelta)
{
	return m_pRendererCom->Add_GameObject_ToRenderGroup(RENDER_GROUP::RENDER_EFFECT, this);
}

HRESULT CEffect_Boss_BrokenLaser_Lightning::Render(RENDER_GROUP::Enum eGroup)
{
...
void CEffect_Boss_BrokenLaser_Lightning::Instance_Size(_float TimeDelta, _int iIndex)
{
}

void CEffect_Boss_BrokenLaser_Lightning::Instance_Pos(_float TimeDelta, _int iIndex)
{
...
void CEffect_Boss_BrokenLaser_Lightning::Instance_UV(_float TimeDelta, _int iIndex)
{
...
HRESULT CEffect_Boss_BrokenLaser_Lightning::Ready_InstanceBuffer()
{
	...
	for (_int iIndex = 0; iIndex < iInstanceCount; ++iIndex)
	{
		...
		_vector vRandPos = vPos + (vRandDir * _float(rand() % 5 + 1) / 10.f);
		XMStoreFloat4(&m_pInstanceBuffer_STT[iIndex].vPosition, vRandPos);

		vRandDir = XMLoadFloat3(&__super::Get_Dir_Rand(_int3(100, 100, 100)));
		_float3 v3RandDir;
...
_float2 CEffect_Boss_BrokenLaser_Lightning::Get_Rand_Size()
{
...
void CEffect_Boss_BrokenLaser_Lightning::Check_InstanceBuffer(_double TimeDelta)
{
	_int iInstanceCount = m_EffectDesc_Prototype.iInstanceCount;
	for (_int iIndex = 0; iIndex < iInstanceCount; ++iIndex)
	{
...
void CEffect_Boss_BrokenLaser_Lightning::Check_TargetMatrix()
{
...
CEffect_Boss_BrokenLaser_Lightning * CEffect_Boss_BrokenLaser_Lightning::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext, void * pArg)
{
	CEffect_Boss_BrokenLaser_Lightning*	pInstance = new CEffect_Boss_BrokenLaser_Lightning(pDevice, pDeviceContext);
	if (FAILED(pInstance->NativeConstruct_Prototype(pArg)))
	{
		MSG_BOX("Failed to Create Instance - CEffect_Boss_BrokenLaser_Lightning");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CEffect_Boss_BrokenLaser_Lightning::Clone_GameObject(void * pArg)
{
	CEffect_Boss_BrokenLaser_Lightning* pInstance = new CEffect_Boss_BrokenLaser_Lightning(*this);
	if (FAILED(pInstance->NativeConstruct(pArg)))
	{
		MSG_BOX("Failed to Clone Instance - CEffect_Boss_BrokenLaser_Lightning");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void CEffect_Boss_BrokenLaser_Lightning::Free()
{
