#include "stdafx.h"
#include "..\Public\Effect_WarpGate_Clear.h"

CEffect_WarpGate_Clear::CEffect_WarpGate_Clear(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	: CInGameEffect(pDevice, pDeviceContext)
{
}

CEffect_WarpGate_Clear::CEffect_WarpGate_Clear(const CEffect_WarpGate_Clear & rhs)
	: CInGameEffect(rhs)
{
}

HRESULT CEffect_WarpGate_Clear::NativeConstruct_Prototype(void * pArg)
{
...
_int CEffect_WarpGate_Clear::Tick(_double TimeDelta)
{
...
_int CEffect_WarpGate_Clear::Late_Tick(_double TimeDelta)
{
	return m_pRendererCom->Add_GameObject_ToRenderGroup(RENDER_GROUP::RENDER_EFFECT, this);
}

HRESULT CEffect_WarpGate_Clear::Render(RENDER_GROUP::Enum eGroup)
{
...
void CEffect_WarpGate_Clear::Check_Instance(_double TimeDelta)
{
	_float4 vMyPos;
	...
	{
		m_pInstanceBuffer_STT[iIndex].fTime -= (_float)TimeDelta * 0.33333333f;
		...
		m_pInstanceBuffer_STT[iIndex].vSize.x -= (_float)TimeDelta * 0.33333333f;
		m_pInstanceBuffer_STT[iIndex].vSize.y -= (_float)TimeDelta * 0.33333333f;
...
void CEffect_WarpGate_Clear::Instance_Size(_float TimeDelta, _int iIndex)
{
}

void CEffect_WarpGate_Clear::Instance_Pos(_float TimeDelta, _int iIndex)
{
	_vector vDir = XMLoadFloat3(&m_pInstance_Dir[iIndex]);
	_vector vPos = XMLoadFloat4(&m_pInstanceBuffer_STT[iIndex].vPosition) + vDir * TimeDelta * 8.f * (m_pInstanceBuffer_STT[iIndex].fTime * m_pInstanceBuffer_STT[iIndex].fTime * m_pInstanceBuffer_STT[iIndex].fTime);

	XMStoreFloat4(&m_pInstanceBuffer_STT[iIndex].vPosition, vPos);
}

void CEffect_WarpGate_Clear::Instance_UV(_float TimeDelta, _int iIndex)
{
}

void CEffect_WarpGate_Clear::Reset_Instance(_double TimeDelta, _float4 vPos, _int iIndex)
{
}

HRESULT CEffect_WarpGate_Clear::Ready_InstanceBuffer()
{
...
CEffect_WarpGate_Clear * CEffect_WarpGate_Clear::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext, void * pArg)
{
	CEffect_WarpGate_Clear*	pInstance = new CEffect_WarpGate_Clear(pDevice, pDeviceContext);
	if (FAILED(pInstance->NativeConstruct_Prototype(pArg)))
	{
		MSG_BOX("Failed to Create Instance - CEffect_WarpGate_Clear");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CEffect_WarpGate_Clear::Clone_GameObject(void * pArg)
{
	CEffect_WarpGate_Clear* pInstance = new CEffect_WarpGate_Clear(*this);
	if (FAILED(pInstance->NativeConstruct(pArg)))
	{
		MSG_BOX("Failed to Clone Instance - CEffect_WarpGate_Clear");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void CEffect_WarpGate_Clear::Free()
{
