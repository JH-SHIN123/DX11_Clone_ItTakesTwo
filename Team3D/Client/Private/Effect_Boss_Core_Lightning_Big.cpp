#include "stdafx.h"
#include "..\Public\Effect_Boss_Core_Lightning_Big.h"

CEffect_Boss_Core_Lightning_Big::CEffect_Boss_Core_Lightning_Big(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	: CInGameEffect(pDevice, pDeviceContext)
{
}

CEffect_Boss_Core_Lightning_Big::CEffect_Boss_Core_Lightning_Big(const CEffect_Boss_Core_Lightning_Big & rhs)
	: CInGameEffect(rhs)
{
}

HRESULT CEffect_Boss_Core_Lightning_Big::NativeConstruct_Prototype(void * pArg)
{
...
HRESULT CEffect_Boss_Core_Lightning_Big::NativeConstruct(void * pArg)
{
	if (nullptr != pArg)
...
_int CEffect_Boss_Core_Lightning_Big::Tick(_double TimeDelta)
{
...
_int CEffect_Boss_Core_Lightning_Big::Late_Tick(_double TimeDelta)
{
	return m_pRendererCom->Add_GameObject_ToRenderGroup(RENDER_GROUP::RENDER_EFFECT, this);
}

HRESULT CEffect_Boss_Core_Lightning_Big::Render(RENDER_GROUP::Enum eGroup)
{
...
void CEffect_Boss_Core_Lightning_Big::Instance_Size(_float TimeDelta, _int iIndex)
{
}

void CEffect_Boss_Core_Lightning_Big::Instance_Pos(_float TimeDelta, _int iIndex)
{
...
void CEffect_Boss_Core_Lightning_Big::Instance_UV(_float TimeDelta, _int iIndex)
{
}

void CEffect_Boss_Core_Lightning_Big::Instance_UV_Custom(_float TimeDelta, _vector vPos, _int iIndex)
{
	...
	if (0.0 >= m_pInstance_UVTime[iIndex])
	{
		...
		m_pInstanceChangeUV_Count[iIndex]++;
	}

	if (10 == m_pInstanceChangeUV_Count[iIndex])
	{
		m_pInstanceChangeUV_Count[iIndex] = 0;
...
HRESULT CEffect_Boss_Core_Lightning_Big::Ready_InstanceBuffer()
{
	...
	m_pInstanceChangeUV_Count	= new _int[iInstanceCount];
...
_float2 CEffect_Boss_Core_Lightning_Big::Get_Rand_Size()
{
...
void CEffect_Boss_Core_Lightning_Big::Check_InstanceBuffer(_double TimeDelta)
{
	_vector vPos = m_pTransformCom->Get_State(CTransform::STATE_POSITION);
	...
	for (_int iIndex = 0; iIndex < iInstanceCount; ++iIndex)
	{
		...
		if (0.0 <= m_pInstance_Pos_UpdateTime[iIndex])
			Instance_UV_Custom((_float)TimeDelta, vPos, iIndex);
	}
}

CEffect_Boss_Core_Lightning_Big * CEffect_Boss_Core_Lightning_Big::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext, void * pArg)
{
	CEffect_Boss_Core_Lightning_Big*	pInstance = new CEffect_Boss_Core_Lightning_Big(pDevice, pDeviceContext);
	if (FAILED(pInstance->NativeConstruct_Prototype(pArg)))
	{
		MSG_BOX("Failed to Create Instance - CEffect_Boss_Core_Lightning_Big");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CEffect_Boss_Core_Lightning_Big::Clone_GameObject(void * pArg)
{
	CEffect_Boss_Core_Lightning_Big* pInstance = new CEffect_Boss_Core_Lightning_Big(*this);
	if (FAILED(pInstance->NativeConstruct(pArg)))
	{
		MSG_BOX("Failed to Clone Instance - CEffect_Boss_Core_Lightning_Big");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void CEffect_Boss_Core_Lightning_Big::Free()
{
	Safe_Release(m_pPointInstanceCom_STT);

	Safe_Delete_Array(m_pInstanceChangeUV_Count);
