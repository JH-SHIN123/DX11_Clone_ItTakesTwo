...
HRESULT CEffect_RespawnTunnel::Render(RENDER_GROUP::Enum eGroup)
{
	...
	m_pBufferRectCom_Preview->Set_ShaderResourceView("g_DiffuseTexture",	m_pTexturesCom_Preview->Get_ShaderResourceView(m_iStageViewer));
...
void CEffect_RespawnTunnel::Set_Stage_Viewer(CWarpGate::STAGE_VALUE eValue)
{
	switch (eValue)
	{
