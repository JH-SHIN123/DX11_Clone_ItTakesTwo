#include "stdafx.h"
#include "..\Public\Effect_StarBuddy_Explosion_Particle.h"

CEffect_StarBuddy_Explosion_Particle::CEffect_StarBuddy_Explosion_Particle(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	: CInGameEffect(pDevice, pDeviceContext)
{
}

CEffect_StarBuddy_Explosion_Particle::CEffect_StarBuddy_Explosion_Particle(const CEffect_StarBuddy_Explosion_Particle & rhs)
	: CInGameEffect(rhs)
{
}

HRESULT CEffect_StarBuddy_Explosion_Particle::NativeConstruct_Prototype(void * pArg)
{
...
HRESULT CEffect_StarBuddy_Explosion_Particle::NativeConstruct(void * pArg)
{
	if (nullptr != pArg)
...
_int CEffect_StarBuddy_Explosion_Particle::Tick(_double TimeDelta)
{
...
_int CEffect_StarBuddy_Explosion_Particle::Late_Tick(_double TimeDelta)
{
	return m_pRendererCom->Add_GameObject_ToRenderGroup(RENDER_GROUP::RENDER_EFFECT, this);
}

HRESULT CEffect_StarBuddy_Explosion_Particle::Render(RENDER_GROUP::Enum eGroup)
{
	...
	m_pPointInstanceCom_STT->Set_ShaderResourceView("g_SecondTexture", m_pTexturesCom_Second->Get_ShaderResourceView((_uint)m_EffectDesc_Clone.fSizePower));
...
void CEffect_StarBuddy_Explosion_Particle::Check_Instance(_double TimeDelta)
{
	_float4 vMyPos;
...
void CEffect_StarBuddy_Explosion_Particle::Instance_Size(_float TimeDelta, _int iIndex)
{
...
void CEffect_StarBuddy_Explosion_Particle::Instance_Pos(_float TimeDelta, _int iIndex)
{
	_vector vDir = XMLoadFloat3(&m_pInstance_Dir[iIndex]);
	_vector vPos = XMLoadFloat4(&m_pInstanceBuffer_STT[iIndex].vPosition) + vDir * TimeDelta * 25.f * (m_pInstanceBuffer_STT[iIndex].fTime * m_pInstanceBuffer_STT[iIndex].fTime + 0.5f) ;
...
void CEffect_StarBuddy_Explosion_Particle::Instance_UV(_float TimeDelta, _int iIndex)
{
}

void CEffect_StarBuddy_Explosion_Particle::Reset_Instance(_double TimeDelta, _float4 vPos, _int iIndex)
{
...
HRESULT CEffect_StarBuddy_Explosion_Particle::Ready_InstanceBuffer()
{
...
_float3 CEffect_StarBuddy_Explosion_Particle::Get_Particle_Rand_Dir(_fvector vDefaultPos)
{
	_vector vRandDir = XMLoadFloat3(&__super::Get_Dir_Rand(_int3(100, 100, 100)));
...
void CEffect_StarBuddy_Explosion_Particle::Check_Target_Matrix()
{
}

_float2 CEffect_StarBuddy_Explosion_Particle::Get_RandSize()
{
	_int iRand = rand() % 3;
...
CEffect_StarBuddy_Explosion_Particle * CEffect_StarBuddy_Explosion_Particle::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext, void * pArg)
{
	CEffect_StarBuddy_Explosion_Particle*	pInstance = new CEffect_StarBuddy_Explosion_Particle(pDevice, pDeviceContext);
	if (FAILED(pInstance->NativeConstruct_Prototype(pArg)))
	{
		MSG_BOX("Failed to Create Instance - CEffect_StarBuddy_Explosion_Particle");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CEffect_StarBuddy_Explosion_Particle::Clone_GameObject(void * pArg)
{
	CEffect_StarBuddy_Explosion_Particle* pInstance = new CEffect_StarBuddy_Explosion_Particle(*this);
	if (FAILED(pInstance->NativeConstruct(pArg)))
	{
		MSG_BOX("Failed to Clone Instance - CEffect_StarBuddy_Explosion_Particle");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void CEffect_StarBuddy_Explosion_Particle::Free()
{
