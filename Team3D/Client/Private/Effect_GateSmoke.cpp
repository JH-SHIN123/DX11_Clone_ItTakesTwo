...
HRESULT CEffect_GateSmoke::NativeConstruct(void * pArg)
{
	...
	XMMatrixDecompose(&vScale, &vQuat, &vPosition, XMLoadFloat4x4(&ArgDesc.WorldMatrix));
	XMStoreFloat4x4(&m_Quat, XMMatrixRotationQuaternion(vQuat));

	m_pTransformCom->Set_WorldMatrix(XMMatrixTranslation(XMVectorGetX(vPosition), XMVectorGetY(vPosition), XMVectorGetZ(vPosition)));
	...
	m_pPosX				= new _float[m_iInstanceCount];

	for (_uint iIndex = 0; iIndex < m_iInstanceCount; ++iIndex)
	{
		m_pAlphaWeight[iIndex] = (rand() % 100) * 0.01f;
		m_pSize[iIndex] = m_pAlphaWeight[iIndex] * 6.f;
		m_pPosX[iIndex] = (rand() % 100) * 0.05f;

		_matrix WorldMatrix;
		
		if (m_pAlphaWeight[iIndex] < 0.15f)
			WorldMatrix = XMMatrixScaling(m_pSize[iIndex], m_pSize[iIndex], m_pSize[iIndex]) * XMMatrixTranslation(m_pPosX[iIndex], 1.5f - m_pAlphaWeight[iIndex] * 10.f, m_pAlphaWeight[iIndex] * 10.f);
		else if (m_pAlphaWeight[iIndex] > 0.8f)
			WorldMatrix = XMMatrixScaling(m_pSize[iIndex], m_pSize[iIndex], m_pSize[iIndex]) * XMMatrixTranslation(m_pPosX[iIndex], m_pAlphaWeight[iIndex] * 10.f - 8.f, m_pAlphaWeight[iIndex] * 10.f);
		else
			WorldMatrix = XMMatrixScaling(m_pSize[iIndex], m_pSize[iIndex], m_pSize[iIndex]) * XMMatrixTranslation(m_pPosX[iIndex], 0.f, m_pAlphaWeight[iIndex] * 10.f);
...
_int CEffect_GateSmoke::Tick(_double dTimeDelta)
{
	...
	for (_uint iIndex = 0; iIndex < m_iInstanceCount; ++iIndex)
	{
		...
		if (m_pAlphaWeight[iIndex] < 0.15f)
			LocalMatrix.r[3] = XMVectorSet(m_pPosX[iIndex], 1.5f - m_pAlphaWeight[iIndex] * 10.f, m_pAlphaWeight[iIndex] * 10.f, 1.f - m_pAlphaWeight[iIndex]);
		else if (m_pAlphaWeight[iIndex] > 0.8f)
			LocalMatrix.r[3] = XMVectorSet(m_pPosX[iIndex], m_pAlphaWeight[iIndex] * 10.f - 8.f, m_pAlphaWeight[iIndex] * 10.f, 1.f - m_pAlphaWeight[iIndex]);
		else
			LocalMatrix.r[3] = XMVectorSet(m_pPosX[iIndex], 0.f, m_pAlphaWeight[iIndex] * 10.f, 1.f - m_pAlphaWeight[iIndex]);

		LocalMatrix.r[3] = XMVector3TransformCoord(LocalMatrix.r[3], XMLoadFloat4x4(&m_Quat));
