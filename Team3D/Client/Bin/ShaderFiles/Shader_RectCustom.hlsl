...
VS_OUT	VS_MAIN(VS_IN In)
{
	...
	Out.WorldMatrix = In.WorldMatrix;
...
void  GS_MAIN_DIST(/*입력*/ triangle  VS_OUT In[3], /*출력*/ inout TriangleStream<GS_OUT_DIST> TriStream)
{
	...
	for (uint i = 0; i < 3; i++)
	{
		...
		matrix matWorld;
		matWorld._11_12_13_14 = vector( vRight.x, vUp.x, vLook.x, In[i].WorldMatrix._41 );
		matWorld._21_22_23_24 = vector( vRight.y, vUp.y, vLook.y, In[i].WorldMatrix._42 );
		matWorld._31_32_33_34 = vector( vRight.z, vUp.z, vLook.z, In[i].WorldMatrix._43 );
		matWorld._41_42_43_44 = vector( 0.f, 0.f, 0.f, 1.f );
	...
	for (uint j = 0; j < 3; j++)
	{
		float3 vScale = { length(In[j].WorldMatrix._11_12_13_14),  length(In[j].WorldMatrix._21_22_23_24) ,  length(In[j].WorldMatrix._31_32_33_34) };
		float3 vLook = normalize(In[j].WorldMatrix._31_32_33_34).xyz;
		float3 vDir = normalize(g_vSubCamPosition - In[j].vPosition).xyz;
		float3 vRight = cross(vLook, vDir).xyz;
		float3 vUp = cross(vRight, vLook).xyz;
		vRight *= vScale.x;
		vUp *= vScale.y;
		...
		matrix matWorld;
		matWorld._11_12_13_14 = vector(vRight.x, vUp.x, vLook.x, In[j].WorldMatrix._41);
		matWorld._21_22_23_24 = vector(vRight.y, vUp.y, vLook.y, In[j].WorldMatrix._42);
		matWorld._31_32_33_34 = vector(vRight.z, vUp.z, vLook.z, In[j].WorldMatrix._43);
		matWorld._41_42_43_44 = vector(0.f, 0.f, 0.f, 1.f);
