...
CVIBuffer_PointInstance_Custom_STT::CVIBuffer_PointInstance_Custom_STT(const CVIBuffer_PointInstance_Custom_STT & rhs)
	: CVIBuffer(rhs)
...
HRESULT CVIBuffer_PointInstance_Custom_STT::NativeConstruct_Prototype(_uint iMaxInstanceCount, const _tchar * pShaderFilePath, const char * pTechniqueName)
{
	...
	/* For.InstanceBuffer */
	m_pInstanceVertices = new VTXMATRIX_CUSTOM_STT[m_iMaxInstanceCount];
	...
	Create_Buffer(&m_pVBInstance, m_iMaxInstanceCount * sizeof(VTXMATRIX_CUSTOM_STT), D3D11_USAGE_DYNAMIC, D3D11_BIND_VERTEX_BUFFER, D3D11_CPU_ACCESS_WRITE, 0, sizeof(VTXMATRIX_CUSTOM_STT), m_pInstanceVertices, false);
...
HRESULT CVIBuffer_PointInstance_Custom_STT::NativeConstruct(void * pArg)
{
...
HRESULT CVIBuffer_PointInstance_Custom_STT::Render(_uint iPassIndex)
{
	MSG_BOX("Wrong Access - CVIBuffer_PointInstance_Custom_STT");

	return S_OK;
}

HRESULT CVIBuffer_PointInstance_Custom_STT::Render(_uint iPassIndex, void * VertexMatrices, _uint iInstanceCount, _uint iStartIndex)
{
	...
	memcpy(MappedSubResource.pData, VertexMatrices, sizeof(VTXMATRIX_CUSTOM_STT) * iInstanceCount);
	m_pDeviceContext->Unmap(m_pVBInstance, 0);

	/* For.RenderBuffer */
	ID3D11Buffer* pBuffers[2] = { m_pVB, m_pVBInstance };
	_uint iStrides[2] = { sizeof(VTXPOS), sizeof(VTXMATRIX_CUSTOM_STT) };
...
CVIBuffer_PointInstance_Custom_STT * CVIBuffer_PointInstance_Custom_STT::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext, _uint iMaxInstanceCount, const _tchar * pShaderFilePath, const char * pTechniqueName)
{
	CVIBuffer_PointInstance_Custom_STT* pInstance = new CVIBuffer_PointInstance_Custom_STT(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype(iMaxInstanceCount, pShaderFilePath, pTechniqueName)))
	{
		MSG_BOX("Failed to Create Instance - CVIBuffer_PointInstance_Custom_STT");
...
CComponent * CVIBuffer_PointInstance_Custom_STT::Clone_Component(void * pArg)
{
	CVIBuffer_PointInstance_Custom_STT* pInstance = new CVIBuffer_PointInstance_Custom_STT(*this);

	if (FAILED(pInstance->NativeConstruct(pArg)))
	{
		MSG_BOX("Failed to Clone Instance - CVIBuffer_PointInstance_Custom_STT");
		Safe_Release(pInstance);
	}
...
void CVIBuffer_PointInstance_Custom_STT::Free()
{
	if (false == m_isClone)
