...
HRESULT CVIBuffer_PointInstance::NativeConstruct_Prototype(_uint iMaxInstanceCount, const _tchar * pShaderFilePath, const char * pTechniqueName)
{
	...
	for (_uint iIndex = 0; iIndex < m_iMaxInstanceCount; ++iIndex)
	{
		...
		pVertices[iIndex].vSize		= _float2(1.f, 1.f);
	}

	Store_VertexBufferInfo(2, m_iMaxInstanceCount, sizeof(VTXPOINT));
...
{
	MSG_BOX("Wrong Access - CVIBuffer_PointInstance");

	return S_OK;
}

HRESULT CVIBuffer_PointInstance::Render(_uint iPassIndex, void * VertexMatrices, _uint iInstanceCount)
{
	...
	_uint iStrides[2] = { sizeof(VTXPOINT), sizeof(VTXMATRIX) };
...
CVIBuffer_PointInstance * CVIBuffer_PointInstance::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext, _uint iMaxInstanceCount, const _tchar * pShaderFilePath, const char * pTechniqueName)
{
	...
	{
		MSG_BOX("Failed to Create Instance - CVIBuffer_PointInstance");
...
CComponent * CVIBuffer_PointInstance::Clone_Component(void * pArg)
{
	CVIBuffer_PointInstance* pInstance = new CVIBuffer_PointInstance(*this);

	if (FAILED(pInstance->NativeConstruct(pArg)))
	{
		MSG_BOX("Failed to Clone Instance - CVIBuffer_PointInstance");
		Safe_Release(pInstance);
	}
