...
void CFrustum::Transform_ToWorldSpace()
{
	...
	/* For.FullScreen - Main */
	InverseProjMatrix = pPipeline->Get_Transform(CPipeline::TS_FULLSCREENPROJ_INVERSE);
	InverseViewMatrix = pPipeline->Get_Transform(CPipeline::TS_MAINVIEW_INVERSE);
	CombinedMatrix = InverseProjMatrix * InverseViewMatrix;
	...
	{
		vPoints_World[iIndex] = XMVector3TransformCoord(XMLoadFloat3(&m_vPoints[iIndex]), CombinedMatrix);
		XMStoreFloat3(&m_Frustum[FRUSTUM_MAIN_FULLSCREEN].vPoints_World[iIndex], vPoints_World[iIndex]);
	}

	XMStoreFloat4(&m_Frustum[FRUSTUM_MAIN_FULLSCREEN].PlaneWorld[0], XMPlaneFromPoints(vPoints_World[1], vPoints_World[5], vPoints_World[6]));
	XMStoreFloat4(&m_Frustum[FRUSTUM_MAIN_FULLSCREEN].PlaneWorld[1], XMPlaneFromPoints(vPoints_World[4], vPoints_World[0], vPoints_World[3]));
	XMStoreFloat4(&m_Frustum[FRUSTUM_MAIN_FULLSCREEN].PlaneWorld[2], XMPlaneFromPoints(vPoints_World[4], vPoints_World[5], vPoints_World[1]));
	XMStoreFloat4(&m_Frustum[FRUSTUM_MAIN_FULLSCREEN].PlaneWorld[3], XMPlaneFromPoints(vPoints_World[3], vPoints_World[2], vPoints_World[6]));
	XMStoreFloat4(&m_Frustum[FRUSTUM_MAIN_FULLSCREEN].PlaneWorld[4], XMPlaneFromPoints(vPoints_World[5], vPoints_World[4], vPoints_World[7]));
	XMStoreFloat4(&m_Frustum[FRUSTUM_MAIN_FULLSCREEN].PlaneWorld[5], XMPlaneFromPoints(vPoints_World[0], vPoints_World[1], vPoints_World[2]));

	m_Frustum[FRUSTUM_MAIN_FULLSCREEN].fFrustumDepth = XMVectorGetX(XMVector3Length(XMLoadFloat4(&m_Frustum[FRUSTUM_MAIN_FULLSCREEN].PlaneWorld[4]) - XMLoadFloat4(&m_Frustum[FRUSTUM_MAIN_FULLSCREEN].PlaneWorld[5])));
	...
	{
		vPoints_World[iIndex] = XMVector3TransformCoord(XMLoadFloat3(&m_vPoints[iIndex]), CombinedMatrix);
		XMStoreFloat3(&m_Frustum[FRUSTUM_SUB_FULLSCREEN].vPoints_World[iIndex], vPoints_World[iIndex]);
	}

	XMStoreFloat4(&m_Frustum[FRUSTUM_SUB_FULLSCREEN].PlaneWorld[0], XMPlaneFromPoints(vPoints_World[1], vPoints_World[5], vPoints_World[6]));
	XMStoreFloat4(&m_Frustum[FRUSTUM_SUB_FULLSCREEN].PlaneWorld[1], XMPlaneFromPoints(vPoints_World[4], vPoints_World[0], vPoints_World[3]));
	XMStoreFloat4(&m_Frustum[FRUSTUM_SUB_FULLSCREEN].PlaneWorld[2], XMPlaneFromPoints(vPoints_World[4], vPoints_World[5], vPoints_World[1]));
	XMStoreFloat4(&m_Frustum[FRUSTUM_SUB_FULLSCREEN].PlaneWorld[3], XMPlaneFromPoints(vPoints_World[3], vPoints_World[2], vPoints_World[6]));
	XMStoreFloat4(&m_Frustum[FRUSTUM_SUB_FULLSCREEN].PlaneWorld[4], XMPlaneFromPoints(vPoints_World[5], vPoints_World[4], vPoints_World[7]));
	XMStoreFloat4(&m_Frustum[FRUSTUM_SUB_FULLSCREEN].PlaneWorld[5], XMPlaneFromPoints(vPoints_World[0], vPoints_World[1], vPoints_World[2]));

	m_Frustum[FRUSTUM_SUB_FULLSCREEN].fFrustumDepth = XMVectorGetX(XMVector3Length(XMLoadFloat4(&m_Frustum[FRUSTUM_SUB_FULLSCREEN].PlaneWorld[4]) - XMLoadFloat4(&m_Frustum[FRUSTUM_SUB_FULLSCREEN].PlaneWorld[5])));
...
void CFrustum::Transform_ToLocalSpace(_fmatrix WorldMatrix)
{
	...
	// Fullscreen not use in local space (for. shadow)
}
